...
class ArcGISHarvester(BaseHarvester):
    ...
    def add_provenance(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if hub_path and os.path.exists(hub_path):
            ...
            # Guarantee that hub_df contains every column in df so concat will succeed
            # missing_cols = set(df.columns) - set(hub_df.columns)
            # for col in missing_cols:
            #     hub_df[col] = ""

            # Preserve column order of the main dataframe
            # hub_df = hub_df[df.columns]
