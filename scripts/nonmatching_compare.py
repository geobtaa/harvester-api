...
def find_unmatched_rows(csv_a_path, csv_b_path, output_csv_path):
    """
    Compares two CSV files based on an 'ID' column and writes the unmatched rows
    ...
    try:
        ...
        # Filter rows in DataFrame A where the 'ID' is NOT in the set of IDs from B.
        # The ~ operator negates the boolean series returned by isin().
        unmatched_df = df_a[~df_a['ID'].isin(ids_in_b)]

        # Check if any unmatched rows were found.
        if unmatched_df.empty:
            print("No unmatched IDs found. All IDs in CSV-A are present in CSV-B.")
            print("Output file will not be created.")
        else:
            # Write the resulting DataFrame of unmatched rows to a new CSV file.
            # index=False prevents pandas from writing the DataFrame index as a column.
            unmatched_df.to_csv(output_csv_path, index=False)
            print(f"Successfully created {output_csv_path} with {len(unmatched_df)} unmatched rows.")
...
find_unmatched_rows(metadata_csv, id_list_csv, output_csv)
